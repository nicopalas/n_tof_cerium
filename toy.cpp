...
void toy(int run_number, float threshold, double time_for_coincidence) {
    ...
    for (auto &sig : signals) {
        ...
        for (auto &conf : configurations) {
            for (auto &c_sig : conf) {
                if (sig.RunNumber == c_sig.RunNumber &&
                    sig.time == c_sig.time &&
                    sig.psTime == c_sig.psTime &&
                    sig.BunchNumber == c_sig.BunchNumber &&
                    sig.PSpulse == c_sig.PSpulse &&
                    sig.PulseIntensity == c_sig.PulseIntensity &&
                    std::abs(sig.tof - c_sig.tof) < time_for_coincidence) {
                    conf.push_back(sig);
                    sig.used = true;
                    merged = true;
                    merged_signals++;
            ...
            if (merged) break; // no need to check other configurations once merged
        }
        if (!merged) { // not merged, mark as used anyway because it does not form coincidence
            sig.used = true;
        }
    }
    ...
    // merging overlapping coincidences
    int merge_operations = 0;
    for (size_t i = 0; i < configurations.size(); ++i) {
        for (size_t j = i + 1; j < configurations.size();) {
            bool overlap = false;
            for (auto &s_i : configurations[i]) {
                for (auto &s_j : configurations[j]) {
                    if (s_i.RunNumber == s_j.RunNumber &&
                        s_i.time == s_j.time &&
                        s_i.psTime == s_j.psTime &&
                        s_i.BunchNumber == s_j.BunchNumber &&
                        s_i.PSpulse == s_j.PSpulse &&
                        s_i.PulseIntensity == s_j.PulseIntensity &&
                        std::abs(s_i.tof - s_j.tof) < time_for_coincidence) {
                        overlap = true;
                        break;
                    }
                }
                if (overlap) break; // if already found overlap, no need to check further inside the same j
            }
            if (overlap) { // merge j into i
                configurations[i].insert(configurations[i].end(), configurations[j].begin(), configurations[j].end());
                configurations.erase(configurations.begin() + j);
                merge_operations++;
            } else ++j;
        }
    }
