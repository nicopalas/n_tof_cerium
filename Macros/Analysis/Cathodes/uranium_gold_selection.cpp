...
int main(){
    ...
    // define variables for the trees
    ...
    for (Int_t i= 0 ; i<nentries; i++){
        ...
        for (int j = 0; j<multA7; j++){
            for (int k = 0; k<multA8;k++){
                ...
                if (multA7>0 && multA8>0 && ampA7[j]>10000 && ampA8[k]>2000 && tofA7[j]-tofA8[k]<3  && abs(tofA7[j]-tofA8[k])<10 && ampA7[j]+ampA8[k]>16000 && tofA7[j]>-650){ // amplitude and tof cuts  
                    // filling
                    tof7_gold = tofA7[j];
                    tof8_gold = tofA8[k];
                    ampA7_gold = ampA7[j];
        ...
        for (int j = 0 ; j<multA8 ; j++){
            for (int k = 0; k<multA9; k++){
                ...
                if (multA8>0 && multA9>0 && abs(tofA9[k]-tofA8[j])<10 && ampA9[k]+ampA8[j]>9500 && tofA8[j]>-650){
