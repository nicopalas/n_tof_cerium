...
int main() {
    ...
    // add new branches to existing tree
    ...
    for (Long64_t i = 0; i < n_gold; i++) {
        ...
        x7_corr = x7 - (mean_x7 + 25.0);
        y7_corr = y7 - mean_y7;

        x8_corr = x8 - (mean_x8 - 25.0);
        y8_corr = y8 - mean_y8;

        cos_theta_gold =
            ((x8_corr - x7_corr - 50.) * 0.7071 + 50.0 * 0.7071) /
            TMath::Sqrt(
                TMath::Power(y8_corr - y7_corr, 2) +
                TMath::Power(x8_corr - x7_corr - 50.0, 2) +
                TMath::Power(50.0, 2)
            );

        phi_gold = TMath::ATan2(
            (y8_corr - y7_corr),
            (x8_corr - x7_corr + 50.0) * 0.7071 + 50.0 * 0.7071
        );
    ...
    double cos_theta_det_u, phi_det_u;
    ...
    tout_u->Branch("cos_theta_det", &cos_theta_det_u, "cos_theta_det/D");
    tout_u->Branch("phi_det", &phi_det_u, "phi_det_u/D");
    ...
    for (Long64_t i = 0; i < n_u; i++) {
        ...
        x8_corr_u = x8_u-(mean_x8_u+25);
        y8_corr_u = y8_u;

        x9_corr_u = x9_u-(mean_x9_u-25);
        y9_corr_u = y9_u;

        cos_theta_u =
            ((x9_corr_u - x8_corr_u - 50.0) * 0.7071 + 50.0 * 0.7071) /
            TMath::Sqrt(
                TMath::Power(y9_corr_u - y8_corr_u, 2) +
                TMath::Power(x9_corr_u - x8_corr_u - 50.0, 2) +
                TMath::Power(50.0, 2)
            );
        ...
        // compute also detector angles 

        cos_theta_det_u = 1/TMath::Sqrt(2)*(sin(acos(cos_theta_u))*cos(phi_u)+cos_theta_u);

        phi_det_u = TMath::ATan2(TMath::Sqrt(2)*sin(acos(cos_theta_u))*sin(phi_u), sin(acos(cos_theta_u))*cos(phi_u));
