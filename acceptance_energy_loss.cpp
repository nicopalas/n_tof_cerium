...
const double target_thickness_cm = 3.78e-4; // cm
...
int main(){
    ...
    const char* outfile = "fission_events_output_5MeV.root";
    ...
    for (Long64_t i=0; i<nEntries; ++i){ // loop over each fission event
        ...
        int forward_Z = frag1_forward ? Z1 : Z2;
        int forward_A = frag1_forward ? A1post : A2post;
        int backward_Z = frag1_forward ? Z2 : Z1;
        int backward_A = frag1_forward ? A2post : A1post;
        ...
        double theta = acos(u);
        double cos_theta_val = cos(theta);
        ...
        if (!ok_front || !ok_back) continue;
        if (!in_detector(hit_front, true) || !in_detector(hit_back, false)) continue;
        ...
        double loss_mylar_anode_b = 0; loss_mylar_cathode_y_b = 0; loss_mylar_cathode_x_b = 0; loss_gas_cathode_x_b = 0; loss_gas_cathode_y_b = 0;

        // Forward fragment: UO2 -> Al -> Gas-> Mylar_cathode_x-> Gas-> Mylar_anode-> Gas-> Mylar_cathode_y
        
        // 1. UO2 target
        if (E_forward_final_mevu > 0.01) {
        ...
        if (E_forward_final_mevu <= 0.01 || E_backward_final_mevu <= 0.01) {
            stopped++;
            continue;
        }
        ...
        loss_mylar_anode_b = loss_mylar_anode_b;
        ...
        forward_Z = forward_Z; forward_A = forward_A;
        backward_Z = backward_Z; backward_A = backward_A;
        ...
        double total_tof_forward = tof_target_forward + tof_backing_forward + tof_gas_forward;
        ...
        double total_tof_backward = tof_target_backward + tof_gas_backward;
        tof_diff = total_tof_forward - total_tof_backward;
        // Store positions
        ...
        tout->Fill();
        cout << "Processed: " << accepted << "/" << nEntries << " (" 
             << 100.0*accepted/nEntries << "% accepted, " << stopped << " stopped)\r" << flush;
    }
